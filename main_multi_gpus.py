...
def validate(dataloader,
    ...
    for batch_id, data in enumerate(dataloader):
        ...
        master_loss_meter.update(master_loss, master_batch)
        master_acc1_meter.update(master_acc1, master_batch)
        master_acc5_meter.update(master_acc5, master_batch)
...
def main_worker(*args):
    ...
    if not config.EVAL:
        ...
        dataloader_train = get_data_loader(config, dataset_train, True, True)
        total_batch_train = len(dataloader_train)
        message = f'----- Total # of train batch (single gpu): {total_batch_train}'
        write_log(local_logger, master_logger, message)

    dataset_val = args[2]
    dataloader_val = get_data_loader(config, dataset_val, False, True)
    ...
    for epoch in range(last_epoch + 1, config.TRAIN.NUM_EPOCHS + 1):
        ...
        # Evaluation (optional)
        if epoch % config.REPORT_FREQ == 0 or epoch == config.TRAIN.NUM_EPOCHS:
