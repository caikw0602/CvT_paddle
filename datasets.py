...
class ImageNet2012Dataset(Dataset):
    ...
    This class gets train/val imagenet datasets, which loads transformed data and labels.
    Note:
...
def get_train_transforms(config):
    ...
    aug_op_list.append(
        transforms.RandomResizedCrop((config.DATA.IMAGE_SIZE, config.DATA.IMAGE_SIZE),
                                     scale=(.08, 1.0), interpolation='bicubic')
    )
...
def get_val_transforms(config):
    ...
    transforms_val = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=config.DATA.IMAGENET_MEAN,
                             std=config.DATA.IMAGENET_STD)
    ])
...
def get_data_loader(config, dataset, is_train=True, use_dist_sampler=False):
